...
class PPOBuffer:
    ...
    def __init__(self, int_act_dim: int, size: int, gamma=0.99, lam=0.95) -> None:
        self.obs_buf: List[Optional[ObservationType]] = [None] * size # observation buffer
        ...
        self.next_obs_buf: List[Optional[ObservationType]] = [None] * size # next observation buffer
        self.term_buf = np.zeros(size, dtype=np.bool) # termination buffer
        ...
        self.adv_buf = np.zeros(size, dtype=np.float32) # advantage function buffer
    ...
    def store(self, obs: ObservationType, act: np.ndarray, reward: float, next_obs: ObservationType, terminal: bool,
        ...
        self.ptr += 1 # add one to the pointer
    ...
    def finish_path(self, last_val: float) -> float:
        ...
        self.adv_buf[path_slice] = util.discount_cumsum(deltas, self.gamma * self.lam) #Equivalent to (16) in GAE Paper
